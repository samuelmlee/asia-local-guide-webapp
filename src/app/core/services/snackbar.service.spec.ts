...
describe('SnackbarService', () => {
  ...
  beforeEach(() => {
    ...
    service = new SnackbarService(matSnackBarSpy);
  });
  ...
  describe('openSuccess', () => {
    it('should call snackBar.open with success message', () => {
      ...
      expect(matSnackBarSpy.open).toHaveBeenCalledWith(message, '', {
        duration: 3000,
    ...
    it('should use empty string as action', () => {
      ...
      const actionParam = matSnackBarSpy.open.calls.mostRecent().args[1];
      expect(actionParam).toBe('');
    });

    it('should set duration to 3000ms', () => {
      service.openSuccess('Success');
      const config = matSnackBarSpy.open.calls.mostRecent().args[2];
  ...
  describe('openError', () => {
    it('should call snackBar.open with error message', () => {
      ...
      expect(matSnackBarSpy.open).toHaveBeenCalledWith(message, 'Fermer', {
